...
\begin{itemize}
\item \textbf{Rate.} Given a quantity that varies, you can say what its rate of change measures, in what units, and what a particular value of that rate tells you about the situation --- whether the quantity is given by a formula, a graph, a table, or only words.
\item \textbf{Accumulation.} You can recognize when a question is asking what has piled up rather than how fast something is going, explain what a definite integral adds up and in what units, and set up the integral that answers the question.
\item \textbf{The connection.} You can explain why rate and accumulation are inverse problems, state both Fundamental Theorems in your own words, and say what each one lets you do that the other doesn't.
\item \textbf{Approximation and error.} For any of these quantities you can produce an over-estimate and an under-estimate, bound your error, and say what you would do to bring the error below a stated tolerance.
\item \textbf{Reasoning and communication.} You can decide whether a theorem's hypotheses are met and what follows when they aren't; produce examples and counterexamples to order; find the first wrong step in a flawed argument and repair it; and write an explanation another student in the course could read and follow.
\end{itemize}
...
\item \textbf{Understand:} Demonstrate on our exams and in an individual oral conversation that you can use the ideas of the course and say what they mean.
\item \textbf{Synthesize and communicate:} Put it all together on written assignments and write well-communicated solutions.
\end{itemize}
...
The Mathematics \& Statistics Department also hosts drop-in tutoring in the evenings; days, times, and location TBD. Upperclass tutors will be available to clarify concepts and help you with homework problems.
...
Learning when a computational tool is the right instrument --- and when reaching for it costs you the understanding you came here for --- is itself part of a modern calculus course, and we will talk about it explicitly.
...
For large language models (``AI'' chatbots), the line is between using one to \emph{learn} and using one to \emph{produce}:
\begin{itemize}
\item \textbf{Fine:} asking for a concept to be explained a different way, asking
for extra practice problems, or checking an answer you have already worked out
yourself.
\item \textbf{Not fine:} asking it to solve or draft an assigned problem, whether
before or after your own attempt.
\end{itemize}
If you used one at all, say so at the end of your submission and include your
prompts. Undisclosed use is an Honor Principle violation. And be prepared to
explain any work you submit, verbally and without advance notice --- work you
cannot explain receives no credit.
